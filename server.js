...
function loadData() {
  ...
  return { users: stored.users || [], friendships: stored.friendships || [], messages: stored.messages || [], posts: stored.posts || [] };
}
...
if (!data.users.some((user) => user.username === (process.env.ADMIN_USER || 'admin'))) {
  const credentials = hashPassword(process.env.ADMIN_PASSWORD || 'admin1234');
...
async function api(request, response, pathname) {
  try {
    ...
    if (request.method === 'GET' && pathname === '/api/posts') {
      const posts = data.posts.slice().reverse().map((post) => ({ id: post.id, author: data.users.find((candidate) => candidate.id === post.authorId)?.username || 'unknown', text: decryptMessage(post), createdAt: post.createdAt }));
    ...
    if (request.method === 'POST' && pathname === '/api/posts') {
      ...
      if (!text || text.length > 1000) return send(response, 400, { error: 'Posts must be between 1 and 1,000 characters.' });
      const post = { id: id(), authorId: user.id, createdAt: new Date().toISOString(), ...encryptMessage(text) }; data.posts.push(post); saveData();
      return send(response, 201, { post: { id: post.id, author: user.username, text, createdAt: post.createdAt } });
    }
    ...
    if (request.method === 'GET' && pathname === '/api/admin/overview') {
      ...
      const latestMessage = data.messages.reduce((latest, message) => !latest || message.createdAt > latest.createdAt ? message : latest, null);
      return send(response, 200, { users: data.users.length, messages: data.messages.length, connections: data.friendships.length, lastActivity: latestMessage?.createdAt || null });
    }
